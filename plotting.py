...
to_process = {
    ...
    #},
    'NN_SpamData': { # Rename to match your output type... e.g.  NN_OUTPUT/NN_Culled_GA...  grab the stuff before _GA
        'path': 'NN_OUTPUT',
        'nn_curve': True,
        'multiple_trials': False
    },
        ...
        for i, column_prefix in enumerate(column_prefixes):
            if not nn_curve:
                ...
                plt.fill_between(data.index, mean - std,
                                 mean + std, alpha=0.2)
                plt.plot(data.index, mean, '-', linewidth=3, markersize=3,
                         label=column_prefix)
            else:
                ...
                if not validate_only:
                    plt.plot(data.index, train, '-', linewidth=3, markersize=3,
                             label='Train {} {}'.format(column_prefix, y_label))
                    plt.plot(data.index, val, '-', linewidth=3, markersize=3,
                             label='CV {} {}'.format(column_prefix, y_label))
                else:
                    plt.plot(data.index, test, '-', linewidth=3, markersize=3,
    ...
    for samples in mimic_files.keys():
        ...
        for keep in mimic_files[samples].keys():
            ...
            if nn_curve:
                ...
                main_df = main_df[main_df.index <= 1995]
    ...
    for pop in ga_files.keys():
        ...
        for i, mate in enumerate(ga_files[pop].keys()):
            ...
            if nn_curve:
                ...
                main_df[mate] = main_df[mate][main_df[mate].index <= 1995]
